...
def my_directed_configuration_model(input_G, weight=None,weight_value=None,
    ...
    in_degree_sequence = [x[1] for x in input_G.in_degree()]
    out_degree_sequence = [x[1] for x in input_G.out_degree()]
...
def my_weighted_directed_configuration_model(G, weight='weight', n_chunks=100, mode='quantiles'):
    ...
    edges = []
    for tup in sorted(G.edges(data=True), key=lambda tup : tup[2][weight]):
        edges.append((tup[0],tup[1],{weight:tup[2][weight]}))
    ...
    for chunk in chunks:
        ...
        for u, v, d in list(thisCM.edges(data=True)):
